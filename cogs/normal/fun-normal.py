...
async def имя(self, context: Context) -> None:
	...
	if buttons.choice == "Алиса":
		embed = disnake.Embed(
			description=f"Женское имя Алиса является сокращённой формой имени Adelaide (фр. Adelaide), в свою очередь представляющего собой французский вариант древнегерманского имени Adalheid (Adelheid, Adelheidis). 
	...
	if buttons.choice == "Тимур":
		embed = disnake.Embed(
			description=f"Мужское имя Тимур по происхождению монгольское.
			Значение трактуется как «железо». Хотя есть версия, по которой это имя могло произойти от татарского имени Дамир, 
			от которого произошли и многие другие известные национальные имена.
			К слову, мужское имя Тимур сегодня пользуется неимоверной популярностью в нашей необъятной стране, но что самое главное, 
			так это то, что оно имеет еще и хорошую значимость, и сильную энергетику.
